...
3. 開始建 table，id 通常是 primary key，可在資料型態上標註為 `primary key, UUID`。同個 table 其他相關的 id 叫做 foreign key，比如 `user_id` 可在資料型態上標註為 `reference user.id` 之類的
...
6. 或是多種 id 相關，也可以切出另一張 table 專門描述關聯。比如問卷回饋 table，可能底下有四個 key: `id`, `user_id`, `survey_id`, `product_id`
