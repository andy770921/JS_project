...
import { switchCategoryOrderBy, trackEvent } from '@src/actions';
import { ORDER_BY_OPTIONS, TRACK_EVENT } from '@src/pages/common/constants/shopCategory';
...
const OrderBySelect: FC = () => {
    const orderBy = useSelector((state: RootState) => state.shopCategory.orderBy);
    const isShowCurator = useSelector((state: RootState) => state.shopCategory.isShowCurator);
    ...
    const handleOrderByChange = useCallback(
        (selectedItem: Item) => {
            dispatch(
                trackEvent({
                    ...TRACK_EVENT.PRODUCT_BLOCK.ORDER_BY,
                    eventLabel: selectedItem.value,
                })
            );
            dispatch(switchCategoryOrderBy(selectedItem.value));
        },
        [dispatch]
    );

    const items = ORDER_BY_OPTIONS.map<Item>(({ label, value }) => ({ label: I18n.t(label), value })).filter(
        item => isShowCurator || item.value !== 'Curator'
    );
